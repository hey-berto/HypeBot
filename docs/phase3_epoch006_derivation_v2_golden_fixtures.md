...
| C2 — entry exactly at formal cutoff | In-population, non-excluded strategy trade has `entry_time == formal_cutoff`, non-null entry, and status OPEN | triggered count increments by one under the frozen `entry_time <= formal_cutoff` actual-executed-entry rule; pair remains right-censored and contributes no numeric return | strict `< formal_cutoff`, closed-only counting, or MTM inclusion |
