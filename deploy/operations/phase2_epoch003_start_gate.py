...
def check_identity(
    ...
    *,
    ...
    with sqlite3.connect(uri, uri=True) as db:
        ...
        if len(rows) != 1:
            raise RuntimeError("expected exactly one epoch003 manifest")
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(prog="phase2-epoch003-start-gate")
    for name in (
        "repo",
        "expected-commit",
        "expected-branch",
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"phase2 epoch003 startup gate failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
