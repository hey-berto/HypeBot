...
def validate_phase2_database_path(
    path: str | Path,
    workspace_root: str | Path,
    *,
    allowed_data_root: str | Path | None = None,
) -> Path:
    """Require Phase 2 databases to live inside an explicitly allowed data root.
    ...
    data_root = (
        Path(allowed_data_root).resolve() if allowed_data_root is not None else root
    )
    target = Path(path)
    ...
    if data_root not in target.parents:
        raise IsolationViolation(
            "Phase 2 database must remain inside the explicitly allowed data root"
        )
...
def connect_phase2(
    ...
    *,
    ...
    target = validate_phase2_database_path(
        path, workspace_root, allowed_data_root=allowed_data_root
    )
